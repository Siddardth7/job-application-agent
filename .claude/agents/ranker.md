...
name: ranker
description: >
  Stage 1 of the daily run (2026-08-13 rewire). Reads your HAND-PICKED JD .md files in
...
## Read (only these)
- `JDs/<today>/*.md` — **your input: the JD files you hand-picked and dropped in today's folder.**
...
- `seen_jobs.csv` / `tracker_data.json` — to skip anything already applied/dropped (P1_06 §2 step 1, gate #8).
...
1. **Read every `.md` in `JDs/<today>/`** — one posting each, full JD text already present. No fetching,
   ...
   skip it and note it. Dedup against `seen_jobs.csv` / `tracker_data.json` (P1_06 §2 step 1 / gate #8).
...
3. **Hard gates** (P1_06 §3 / §7): visa/ITAR/EAR/US-person/clearance/export-control, explicit
   ...
   → gate the row, stop scoring it, record the reason. *(Freshness gate #7 is your call now — he
...
6. **Route** (P1_06 §5 — single direct-apply model): **≥50 AND genuine QE/process/QMS role → DIRECT-APPLY**
   ...
   Apply row scoring **≥60** (advisory — you network manually, post-apply). **No referral lane, no
   `referral-needed`.**
...
- Set `networking: recommended` on every Apply row ≥60 (advisory; you network manually — no agent).
- Carry the source `JD file` path on every row so the Customiser and you can trace each score to its JD.

## NEVER
- Never fetch, curl, or pull a JD — the JD files are already on disk; score only what you put in `JDs/<today>/`.
